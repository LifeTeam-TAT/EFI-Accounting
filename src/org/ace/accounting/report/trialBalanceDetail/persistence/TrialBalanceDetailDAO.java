	...
	public List<TrialBalanceReportDto> findTrialBalanceDetailList(TrialBalanceCriteriaDto dto) {
		...
		try {
			...
			StringBuffer queryString = new StringBuffer(
					"SELECT ACODE,ACNAME,ACTYPE,SUM(mDEBIT),SUM(mCREDIT),SUM(DEBIT),SUM(CREDIT) FROM (");
			...
			queryString.append(" , CASE WHEN (  T.TRANTYPEID ='2' OR T.TRANTYPEID ='3' )  THEN ABS(T." + amountColumn
					+ ") ELSE 0 END AS mDEBIT ");
			queryString.append(" ,CASE WHEN (  T.TRANTYPEID ='1' OR T.TRANTYPEID ='4' )  THEN ABS(T." + amountColumn
			...
			queryString.append(
					"SELECT C.ACTYPE AS ACTYPE, CC.DEPARTMENTID  , C.ACCODE AS ACODE  , C.ACNAME AS ACNAME  ,0 AS mDEBIT,0 AS mCREDIT ");
			...
			/*
			 * 
			 * queryString. append(" SELECT C.ACTYPE AS ACTYPE, CC.DEPARTMENTID "); if
			 * (dto.isGroup()) { if
			 ...
			 * else { queryString.append(" , C.IBSBACODE AS ACODE "); } queryString.
			 * append(" , CASE WHEN ACTYPE IN('A','L') THEN C.ACNAME "); queryString.
			 * append(" ELSE (SELECT ACNAME FROM COA C2 WHERE C2.ID = C.HEADID ) END AS ACNAME "
			 * ); } else { if (dto.getAccountType().equals(TrialBalanceAccountType.Gl_ACODE
			 * )) { queryString.append(" , C.ACCODE AS ACODE "); } else {
			 * queryString.append(" , C.IBSBACODE AS ACODE "); } if (dto.getBranch() ==
			 * null) { queryString.append(" , C.ACNAME AS ACNAME "); } else {
			 ...
			 * queryString. append(" T.TRANTYPEID ='1' OR T.TRANTYPEID ='4' ) ");
			 * queryString.append(" THEN ABS(T." + amountColumn +
			 * ") ELSE 0 END AS mCREDIT ");
			 * 
			 * // FROM COA queryString.append(" ,CASE WHEN ( ");
			 * queryString.append(" (ACTYPE IN ('I','L')  AND CC." + reportBudgetMonth +
			 * " < 0 ) OR "); queryString.append(" (ACTYPE IN ('A','E')  AND CC." +
			 * reportBudgetMonth + " > 0 ) "); queryString.append(" ) THEN ABS(CC." +
			 * reportBudgetMonth + ") ELSE 0 END AS DEBIT ");
			 * queryString.append(" ,CASE WHEN ( ");
			 * queryString.append(" (ACTYPE IN ('I','L')  AND CC." + reportBudgetMonth +
			 * " > 0 ) OR "); queryString.append(" (ACTYPE IN ('A','E')  AND CC." +
			 * reportBudgetMonth + " < 0 ) "); queryString.append(" ) THEN ABS(CC." +
			 * reportBudgetMonth + ") ELSE 0 END AS CREDIT "); queryString.
			 * append(" FROM COA AS C INNER JOIN VW_CCOA AS CC ON C.ID=CC.COAID RIGHT JOIN TLF T ON CC.ID = T.CCOAID WHERE CC.BUDGET=?budget "
			 * ); queryString. append(" AND DATEPART(MONTH,T.SETTLEMENTDATE) =?month");
			 * queryString.append(" AND " + reportBudgetMonth + "<> 0 "); if (dto.isGroup())
			...
			if (objList != null) {
				...
				for (TrialBalanceReportDto reportDto : dtoList) {
					...
					} else if (reportDto.getAcType().equals(AccountType.I)
...
//						reportDto.setmDebit(reportDto.getmDebit().subtract(reportDto.getmCredit()));
						reportDto.setCredit(BigDecimal.valueOf(0.00));
